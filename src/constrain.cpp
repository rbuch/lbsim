                                                        ...
                                                        const size_t numProcs)
{
  ...
  std::exponential_distribution<double> expo(0.35);
  std::normal_distribution<double> normal(15, 5);
...
template<template<typename, typename, typename, typename...> class T, typename O, typename P, typename... A>
void testLB(std::vector<O> objs, std::vector<P> procs, std::string lb_name, A&&... args)
{
  constexpr int dimension = O::dimension;
  T<O, P, Solution<O, P>> strat(std::forward<A>(args)...);
...
int main(int argc, char* argv[])
{
  ...
  //testLB<TreeStrategy::GreedyNorm>(objs, procs, "greedynorm");
  testLB<TreeStrategy::KdLB>(objs, procs, "kd");
  //testLB<TreeStrategy::RKdLB>(objs, procs, "rkd");
