...
class NewRKDNode : BaseKDNode<NewRKDNode<Elem, N, Engine>, Elem>
{
  using base = BaseKDNode<NewRKDNode<Elem, N, Engine>, Elem>;
  using rkdt = NewRKDNode*;
...
public:
  NewRKDNode(const Elem& key, const discr_t discr = random(0, N - 1)) : base(key), discr(discr)
  {
  }
  ...
  static Elem* findMinNorm(rkdt t, const T& x)
  {
    ...
    int count = 0;
    return findMinNormHelper(t, x, nullptr, objNorm, bestNorm, mins, count);

    /*
    auto start = std::chrono::steady_clock::now();
    const auto result = findMinNormHelper(t, x, nullptr, objNorm, bestNorm, mins);
    auto end = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
    std::cout << elapsed.count() << std::endl;

    start = std::chrono::steady_clock::now();
    const auto second = findMinNormHelper(t, x, result, objNorm, bestNorm, mins);
...
private:
  ...
  static Elem* findMinNormHelper(rkdt t, const T& x, Elem* bestObj, const KDFloatType objNorm, KDFloatType& bestNorm,
                                 std::array<KDFloatType, N>& minBounds, int& count)
  {
    if (count >= N * 100)
      return bestObj;
    if (t->left != nullptr)
    {
      bestObj = findMinNormHelper(t->left, x, bestObj, objNorm, bestNorm, minBounds, count);
    }
    ...
    if (t->right != nullptr)
    {
      ...
      {
        bestObj = findMinNormHelper(t->right, x, bestObj, objNorm, bestNorm, minBounds, count);
      }
