...
{
template <typename O, typename P, typename S, bool useVectorLoad>
class FuzzyORB : public Strategy<O, P, S>
{
private:
  struct Bounds
  {
                 ...
                 S& solution, std::vector<Indices>& sortedIndices, const Box& box)
  {
    ...
    const auto splitSides = findSplit(objs, sortedIndices[maxDim], ratio, bgLeft, bgRight);

    auto splitIt = std::find(splitSides.begin(), splitSides.end(), Side::Right);

    size_t splitIndex = numObjs - 1;
    if (splitIt != splitSides.end())
      splitIndex = std::distance(splitSides.begin(), splitIt);
  ...
  std::vector<Side> findSplit(const std::vector<O>& objs, const Indices& sortedPositions,
                              const float ratio, const float bgLeft,
                              const float bgRight, const float middleFraction = 0.2) const
  {
    // if constexpr (!useVectorLoad)
    // {
      std::vector<Side> sides(sortedPositions.size(), Side::Right);
      size_t leftSplitIndex = 0;
      ...
      const float leftTarget = ratio * totalLoad * (1 - middleFraction);
      float leftLoad = 0;
      float nextLeftLoad = 0;
      for (leftSplitIndex = 0; leftSplitIndex < sortedPositions.size(); leftSplitIndex++)
      {
        nextLeftLoad += objs[sortedPositions[leftSplitIndex]].getLoad() + approxBgPerObj;
        if (nextLeftLoad > leftTarget)
        {
          ...
          if (std::abs(nextLeftLoad - leftTarget) < std::abs(leftLoad - leftTarget))
          {
            ...
            sides[leftSplitIndex] = Side::Left;
            leftSplitIndex++;
          }
          break;
        }
      ...
      // rightTarget is the amount of object load we want to assign to the right procs
      size_t rightSplitIndex;
      const float rightTarget = (1 - ratio) * totalLoad * (1 - middleFraction);
      float rightLoad = 0;
      float nextRightLoad = 0;
      for (rightSplitIndex = sortedPositions.size() - 1; rightSplitIndex >= leftSplitIndex; rightSplitIndex--)
      {
        nextRightLoad += objs[sortedPositions[rightSplitIndex]].getLoad() + approxBgPerObj;
        if (nextRightLoad > rightTarget)
        {
          // Decide if split element should go to right or right partition
          if (std::abs(nextRightLoad - rightTarget) < std::abs(rightLoad - rightTarget))
          {
            rightLoad = nextRightLoad;
            rightSplitIndex--;
          }
          break;
        }
    ...
    //     for (int i = 0; i < O::dimension; i++)
    //     {
    //       diff += (leftLoad[i] - leftTargets[i]) * (leftLoad[i] - leftTargets[i]);//std::abs(leftLoad[i] - leftTargets[i]);
    //     }
    ...
    //     }
    //   }
    //   //std::cout << minIndex << " (" << minLoadDiff << ")" << std::endl;
    //   return minIndex;
    // }
  }
...
public:
  ...
  {
    static_assert(O::isPosition, "FuzzyORB must be used with position objects");
    CkAssert(!objs.empty());
    CkAssertMsg(!objs[0].position.empty(),
                "Objects used with FuzzyORBLB must have a valid, non-empty position.");
  ...
  class FuzzyORBVector : public FuzzyORB<O, P, S, true>
  {
  };

  template <typename O, typename P, typename S>
  class FuzzyORBScalar : public FuzzyORB<O, P, S, false>
  {
  };
